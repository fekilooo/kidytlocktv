...
class OpmlParserTest {
    ...
    @Test
    void testBilibiliSpaceUrlParsing() throws Exception {
        String opmlContent = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
                + "<opml version=\"1.0\"><body>"
                + "<outline text=\"國語動漫\" type=\"rss\" "
        ...
        assertEquals("bili:5089070", channels.get(0).getChannelId());
        assertEquals("國語動漫", channels.get(0).getTitle());
    }

    @Test
    void testLegacyBilibiliIdParsing() throws Exception {
        String opmlContent = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
                + "<opml version=\"1.0\"><body>"
                + "<outline text=\"國語動漫\" type=\"rss\" "
                + "xmlUrl=\"https://www.youtube.com/feeds/videos.xml?channel_id=bili:5089070\"/>"
                + "</body></opml>";
