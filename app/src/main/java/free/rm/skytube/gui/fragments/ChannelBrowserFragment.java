...
public class ChannelBrowserFragment extends FragmentEx implements ChannelSubscriber {
	...
	public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		binding.channelSubscribeButton.setOnKeyListener((view, keyCode, event) -> {
			if (event.getAction() != KeyEvent.ACTION_DOWN) {
				return false;
			}
			if (keyCode == KeyEvent.KEYCODE_DPAD_UP || keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
				View search = requireActivity().findViewById(R.id.menu_search);
				if (search != null) {
			...
			if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT) {
				for (int i = 0; i < binding.toolbar.getChildCount(); i++) {
					View toolbarChild = binding.toolbar.getChildAt(i);
					if (toolbarChild.isFocusable()) {
						toolbarChild.setFocusableInTouchMode(true);
						toolbarChild.post(toolbarChild::requestFocus);
						return true;
					}
				}
			}
	...
	private synchronized void initViews() {
		if (channel != null) {
			...
			// subscribe button
			if (channel.isBilibili()) {
				userSubscribed = false;
				binding.channelSubscribeButton.setVisibility(View.GONE);
			} else {
				setSubscribedState(channel.isUserSubscribed());
			}

			if (!channel.isBilibili() && userSubscribed) {
