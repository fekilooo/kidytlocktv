...
public class SubscriptionsFeedFragment extends VideosGridFragment implements GetSubscriptionVideosTaskListener {
    ...
    private synchronized void startRefreshTask() {
        boolean started = FeedUpdateTask.getInstance().start(requireContext());
        if (!started && gridviewBinding != null && gridviewBinding.swipeRefreshLayout != null) {
    ...
    public void onChannelVideoFetchFinish(boolean changes) {
        // Always reload the cached feed once the refresh cycle completes so the UI can
        // leave its loading state even when there are no new videos.
        if (changes || (videoGridAdapter != null && videoGridAdapter.getItemCount() == 0)) {
            refreshFeedFromCache();
        }
    }
