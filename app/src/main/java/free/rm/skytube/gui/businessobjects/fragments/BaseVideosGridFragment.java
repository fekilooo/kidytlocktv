...
public abstract class BaseVideosGridFragment extends TabFragment implements SwipeRefreshLayout.OnRefreshListener {
	...
	public void onResume() {
		...
		if(newUpdateCounter != updateCount) {
			// PlaybackStatusDb already notifies VideoGridAdapter about the changed video. Rebinding
			// the entire grid here makes every thumbnail flash when returning from the player.
	...
	public void onDestroyView() {
		// Keep loaded cards while this Fragment remains on the back stack.
		if (videoGridAdapter != null) {
			videoGridAdapter.setSwipeRefreshLayout(null);
		}
		super.onDestroyView();
	}
