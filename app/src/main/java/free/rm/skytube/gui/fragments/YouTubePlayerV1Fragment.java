...
public class YouTubePlayerV1Fragment extends ImmersiveModeFragment implements MediaPlayer.OnPreparedListener, YouTubePlayerFragmentInterface {
	...
	private void initViews() {
		...
		fragmentBinding.videoView.setOnCompletionListener(mediaPlayer -> playNextVideoFromSameChannel());
	...
	private void startPlaybackForVideo(@NonNull YouTubeVideo video, boolean resumeSavedPosition) {
		...
		if (resumeSavedPosition) {
			new ResumeVideoTask(requireContext(), video.getId(), position -> {
	...
	private void playNextVideoFromSameChannel() {
		if (autoPlayingNextVideo || youTubeVideo == null || youTubeVideo.getChannelId() == null) {
			return;
		}
		...
		compositeDisposable.add(
				Single.fromCallable(() -> findNextVideoFromSameChannel(youTubeVideo))
						.subscribeOn(Schedulers.io())
						.observeOn(AndroidSchedulers.mainThread())
						.subscribe(nextVideo -> {
							if (nextVideo != null && isVisible()) {
							...
							autoPlayingNextVideo = false;
							Logger.e(YouTubePlayerV1Fragment.this,
									"Unable to auto play next channel video: " + err.getMessage(), err);
						})
		);
	}

	private YouTubeVideo findNextVideoFromSameChannel(@NonNull YouTubeVideo currentVideo) throws Exception {
		...
		int currentIndex = -1;
		for (int i = 0; i < channelVideos.size(); i++) {
			if (currentVideo.getId().equals(channelVideos.get(i).getId())) {
				currentIndex = i;
				break;
			}
		}
		...
		if (currentIndex >= 0) {
			for (int i = currentIndex + 1; i < channelVideos.size(); i++) {
				YouTubeVideo candidate = channelVideos.get(i);
				if (!currentVideo.getId().equals(candidate.getId())) {
					return candidate;
				}
			}
		}

		for (YouTubeVideo candidate : channelVideos) {
			if (!currentVideo.getId().equals(candidate.getId())) {
				return candidate;
			}
		}
	...
	private void autoPlayResolvedVideo(@NonNull YouTubeVideo nextVideo) {
		compositeDisposable.add(
				Single.fromCallable(() -> {
							YouTubeVideo resolvedVideo = NewPipeService.get().getDetails(nextVideo.getId());
							if (resolvedVideo.getChannel() == null && nextVideo.getChannel() != null) {
								resolvedVideo.setChannel(nextVideo.getChannel());
							}
							return resolvedVideo;
						})
						.subscribeOn(Schedulers.io())
						.observeOn(AndroidSchedulers.mainThread())
						.subscribe(resolvedVideo -> startPlaybackForVideo(resolvedVideo, false), err -> {
							Logger.e(YouTubePlayerV1Fragment.this,
									"Unable to resolve next video details, using preview object: " + err.getMessage(), err);
	...
	private boolean shouldFallbackToLivePlayback(Throwable throwable) {
		return (youTubeVideo != null && youTubeVideo.isLiveStream())
				|| throwable instanceof StreamExtractException;
	}
