...
public class GridViewHolder extends RecyclerView.ViewHolder implements Serializable {
	...
	GridViewHolder(@NonNull VideoCellBinding binding, MainActivityListener listener,
		...
		binding.getRoot().setOnClickListener(view -> openCurrentCard());
		binding.thumbnailImageView.setOnClickListener(view -> openCurrentCard());
		binding.getRoot().setOnLongClickListener(view -> {
			if (currentCard instanceof YouTubeVideo || currentCard instanceof YouTubeChannel) {
				onOptionsButtonClick(binding.optionsButton);
				return true;
			}
			return false;
		});
		binding.getRoot().setOnFocusChangeListener((view, hasFocus) -> {
			...
			view.setTranslationZ(hasFocus ? 12.0f : 0.0f);
		});
	...
	private void openCurrentCard() {
		if (currentCard instanceof YouTubeVideo) {
			TrendingExposureTracker.getInstance().markClicked(currentCard.getId());
	...
	private void resetRecycledViewState() {
		binding.titleTextView.setVisibility(View.VISIBLE);
		binding.thumbsUpTextView.setVisibility(View.VISIBLE);
		binding.videoDurationTextView.setVisibility(View.VISIBLE);
		binding.channelLayout.setVisibility(View.VISIBLE);
		binding.channelTextView.setVisibility(View.VISIBLE);
		binding.viewsTextView.setVisibility(View.VISIBLE);
		binding.separatorTextView.setVisibility(View.VISIBLE);
		binding.publishDateTextView.setVisibility(View.VISIBLE);
    ...
    private void updateViewsData(@NonNull YouTubeVideo youTubeVideo) {
        ...
        updatePlaybackStatus(youTubeVideo);
    }

    void updatePlaybackStatus() {
        if (currentCard instanceof YouTubeVideo) {
            updatePlaybackStatus((YouTubeVideo) currentCard);
        }
    }

    private void updatePlaybackStatus(@NonNull YouTubeVideo youTubeVideo) {
