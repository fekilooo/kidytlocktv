...
public class OpmlExporter {
    ...
    private static final String BILIBILI_OUTLINE_TEMPLATE = "    <outline text=\"%s\" type=\"rss\" xmlUrl=\"https://space.bilibili.com/%s\" htmlUrl=\"https://space.bilibili.com/%s\" />\n";
    ...
    static String generateOpmlContent(List<YouTubeChannel> channels) {
        ...
        for (YouTubeChannel channel : channels) {
            ...
            if (BilibiliService.isChannelId(channel.getId())) {
                String uid = BilibiliService.rawUid(channel.getId()).replaceAll("[^0-9]", "");
                outline = String.format(
                        BILIBILI_OUTLINE_TEMPLATE, channelTitle, uid, uid);
            } else {
                String channelId = filterXmlSafeChannelId(channel.getId());
                outline = String.format(
                        OUTLINE_TEMPLATE, channelTitle, channelId, channelId);
            }
