...
public class VideoBlocker {
	...
	public List<CardData> filter(List<CardData> videosList) {
		return filter(videosList, false);
	}

	/**
	 * Filters cards based on constraints set by the user.
	 *
	 * @param videosList       Cards list to be filtered.
	 * @param isSearchResults  True when cards originate from search results.
	 *
	 * @return A list of valid cards that fit the user's criteria.
	 */
	public List<CardData> filter(List<CardData> videosList, boolean isSearchResults) {
		...
		final boolean       isChannelAllowListEnabled = settings.isChannelAllowListEnabled();
		final boolean       isSubscribedChannelsOnlyEnabled = settings.isSubscribedChannelsOnlyEnabled();
		final List<ChannelId>  blacklistedChannelIds = isChannelBlacklistEnabled
				? ChannelFilteringDb.getChannelFilteringDb().getDeniedChannelsIdsList() : null;
		final List<ChannelId>  whitelistedChannelIds = isChannelAllowListEnabled
				? ChannelFilteringDb.getChannelFilteringDb().getAllowedChannelsIdsList() : null;
		final Set<ChannelId> subscribedChannelIds = isSubscribedChannelsOnlyEnabled
				? SubscriptionsDb.getSubscriptionsDb().getSubscribedChannelIdsSet() : null;
		...
		for (CardData cardData : videosList) {
			if (cardData instanceof YouTubeVideo) {
				...
				if (!((isChannelBlacklistEnabled && filterByBlacklistedChannels(video, blacklistedChannelIds))
						|| (isChannelAllowListEnabled && filterByWhitelistedChannels(video, whitelistedChannelIds))
						|| (isSubscribedChannelsOnlyEnabled && filterBySubscribedChannels(video, subscribedChannelIds))
			...
			} else if (isSearchResults && isSubscribedChannelsOnlyEnabled && cardData instanceof YouTubeChannel) {
				YouTubeChannel channel = (YouTubeChannel) cardData;
				if (!shouldFilterSearchChannel(channel, subscribedChannelIds)) {
					filteredVideosList.add(channel);
				}
			} else if (isSearchResults && isSubscribedChannelsOnlyEnabled && cardData instanceof YouTubePlaylist) {
				YouTubePlaylist playlist = (YouTubePlaylist) cardData;
				if (!shouldFilterSearchPlaylist(playlist, subscribedChannelIds)) {
					filteredVideosList.add(playlist);
				}
			} else {
	...
	private boolean filterBySubscribedChannels(YouTubeVideo video, Set<ChannelId> subscribedChannelIds) {
		if (filterBySubscribedChannels(video.getChannelId(), subscribedChannelIds)) {
			log(video, FilterType.CHANNEL_SUBSCRIPTIONS, video.getChannelName());
			return true;
		} else {
			return false;
		}
	...
	private boolean filterBySubscribedChannels(ChannelId channelId, Set<ChannelId> subscribedChannelIds) {
		if (channelId != null && channelId.getRawId().startsWith("bili:")) {
			return false;
		}
		return subscribedChannelIds == null || !subscribedChannelIds.contains(channelId);
	}

	private boolean shouldFilterSearchChannel(YouTubeChannel channel, Set<ChannelId> subscribedChannelIds) {
		return settings.isFilterSearchChannelResultsEnabled()
				&& filterBySubscribedChannels(channel.getChannelId(), subscribedChannelIds);
	}

	private boolean shouldFilterSearchPlaylist(YouTubePlaylist playlist, Set<ChannelId> subscribedChannelIds) {
		if (!settings.isFilterSearchPlaylistResultsEnabled()) {
			return false;
		}

		YouTubeChannel playlistChannel = playlist.getChannel();
		if (playlistChannel == null) {
			return true;
		}

		return filterBySubscribedChannels(playlistChannel.getChannelId(), subscribedChannelIds);
	}
