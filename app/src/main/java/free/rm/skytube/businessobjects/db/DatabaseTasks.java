...
public class DatabaseTasks {
    ...
    public static PersistentChannel getChannelOrRefresh(Context context, ChannelId channelId, boolean staleAcceptable) throws NewPipeException {
        ...
        if (BilibiliService.isChannelId(channelId.getRawId())) {
            return getBilibiliChannelOrRefresh(
                    context, db, channelId, persistentChannel, staleAcceptable);
        }
    ...
    private static PersistentChannel getBilibiliChannelOrRefresh(
            Context context,
            SubscriptionsDb db,
            ChannelId channelId,
            @Nullable PersistentChannel persistentChannel,
            boolean staleAcceptable) throws NewPipeException {
        YouTubeChannel cached = persistentChannel != null ? persistentChannel.channel() : null;
        boolean incomplete = cached == null
                || TextUtils.isEmpty(cached.getTitle())
                || TextUtils.isEmpty(cached.getThumbnailUrl())
                || cached.getTitle().equals(channelId.getRawId())
                || cached.getTitle().equals("Bilibili "
                        + BilibiliService.rawUid(channelId.getRawId()));
        boolean stale = cached != null
                && cached.getLastCheckTime() < System.currentTimeMillis() - (24 * 60 * 60 * 1000L);
        boolean needsRefresh = incomplete || (!staleAcceptable && stale);

        if (needsRefresh && SkyTubeApp.isConnected(context)) {
            try {
                YouTubeChannel details = BilibiliService.get()
                        .getChannelDetails(channelId.getRawId());
                return db.cacheChannel(persistentChannel, details);
            } catch (IOException error) {
                if (persistentChannel == null) {
                    throw new NewPipeException(
                            "Getting Bilibili channel details for " + channelId
                                    + " fails: " + error.getMessage(), error);
                }
                Log.e(TAG, "Unable to refresh Bilibili channel " + channelId
                        + ": " + error.getMessage(), error);
            }
        }
        if (cached != null) {
            BilibiliService.get().rememberChannel(cached);
        }
        return persistentChannel;
    }
    ...
    public static Single<List<ChannelView>> getSubscribedChannelView(Context context, @Nullable View progressBar,
        ...
        return Single.fromCallable(() -> {
                    refreshIncompleteBilibiliChannels(context);
                    return new VideoBlocker().filterChannels(SubscriptionsDb.getSubscriptionsDb()
    ...
    private static void refreshIncompleteBilibiliChannels(Context context) {
        if (!SkyTubeApp.isConnected(context)) {
            return;
        }
        SubscriptionsDb db = SubscriptionsDb.getSubscriptionsDb();
        try {
            for (YouTubeChannel channel : db.getSubscribedChannels()) {
                if (!BilibiliService.isChannelId(channel.getId())) {
                    continue;
                }
                try {
                    getChannelOrRefresh(context, channel.getChannelId(), true);
                } catch (NewPipeException error) {
                    Log.e(TAG, "Unable to complete Bilibili subscription metadata for "
                            + channel.getId() + ": " + error.getMessage(), error);
                }
            }
        } catch (IOException error) {
            Log.e(TAG, "Unable to inspect Bilibili subscriptions: " + error.getMessage(), error);
        }
    }
