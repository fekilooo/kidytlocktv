...
public class YouTubeChannel extends CardData implements Serializable {
	...
	public boolean isBilibili() {
		return id != null && id.startsWith("bili:");
	}
	...
	public Single<Boolean> blockChannel(boolean displayToastMessage) {
        if (SkyTubeApp.getSettings().isSubscribedChannelsOnlyEnabled()) {
            return DatabaseTasks.subscribeToChannel(false,
                            null, SkyTubeApp.getContext(), getChannelId(), displayToastMessage)
                    .map(result -> result.second == DatabaseResult.SUCCESS)
    ...
    public Single<Boolean> unblockChannel(boolean displayToastMessage) {
        if (SkyTubeApp.getSettings().isSubscribedChannelsOnlyEnabled()) {
            return DatabaseTasks.subscribeToChannel(true,
                            null, SkyTubeApp.getContext(), getChannelId(), displayToastMessage)
                    .map(result -> result.second.isPositive())
                    .observeOn(AndroidSchedulers.mainThread());
        }
