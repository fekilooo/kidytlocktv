...
public class OpmlParser {
    ...
    private static final Pattern BILIBILI_SPACE_PATTERN = Pattern.compile(
            "(?:https?://)?(?:www\\.)?space\\.bilibili\\.com/(\\d+)",
            Pattern.CASE_INSENSITIVE);
    private static final Pattern BILIBILI_ID_PATTERN = Pattern.compile(
            "(?:^|[?&=/])bili:(\\d+)(?:$|[&#?/])",
            Pattern.CASE_INSENSITIVE);
    ...
    private static @Nullable ParsedChannel parseOutlineTag(XmlPullParser parser) {
        ...
        if (xmlUrl != null) {
            channelId = tryExtractBilibiliChannelId(xmlUrl);
            if (channelId == null) {
        ...
        if (channelId == null && htmlUrl != null) {
            channelId = tryExtractBilibiliChannelId(htmlUrl);
            if (channelId == null) {
    ...
    private static @Nullable String tryExtractBilibiliChannelId(String url) {
        Matcher matcher = BILIBILI_SPACE_PATTERN.matcher(url);
        if (matcher.find()) {
            return "bili:" + matcher.group(1);
        }
        matcher = BILIBILI_ID_PATTERN.matcher(url);
        if (matcher.find()) {
            return "bili:" + matcher.group(1);
        }
        return null;
