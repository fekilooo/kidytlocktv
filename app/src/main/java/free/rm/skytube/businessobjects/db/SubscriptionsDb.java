...
public class SubscriptionsDb extends SQLiteOpenHelperEx {
	...
	public DatabaseResult unsubscribe(PersistentChannel channel) {
        ...
        getWritableDatabase().delete(SubscriptionsVideosTable.TABLE_NAME_V2,
                SubscriptionsVideosTable.COL_SUBS_ID.name() + " = ? OR "
                        + SubscriptionsVideosTable.COL_CHANNEL_PK.name() + " = ?",
                new String[]{String.valueOf(channel.subscriptionPk()),
                        String.valueOf(channel.channelPk())});
    ...
    public Set<ChannelId> getSubscribedChannelIdsSet() {
        return new HashSet<>(getSubscribedChannelIds());
    }
    ...
    public List<YouTubeVideo> getRecentSubscriptionVideos(long minPublishTimestamp) {
        SkyTubeApp.nonUiThread();

        final String query = SubscriptionsVideosTable.BASE_QUERY
                + " WHERE s." + SubscriptionsVideosTable.COL_PUBLISH_TIME.name() + " >= ?"
                + " ORDER BY s." + SubscriptionsVideosTable.COL_PUBLISH_TIME.name() + " DESC";
        try (Stopwatch s = new Stopwatch("getRecentSubscriptionVideos since=" + minPublishTimestamp)) {
            Cursor cursor = getReadableDatabase().rawQuery(query, new String[]{String.valueOf(minPublishTimestamp)});
            return extractVideos(cursor, true).stream()
                    .filter(video -> video.getChannelId() != null)
    ...
    /**
     * Bilibili channels are not resolved by the YouTube extractor. Older builds could therefore
     * mark them as missing during a generic subscription refresh. Restore those stale states so
     * the channel remains a normal local subscription.
     */
    private void repairBilibiliChannelStates() {
        ContentValues values = new ContentValues();
        values.put(LocalChannelTable.COL_STATE.name(), Status.OK.code);
        int updated = getWritableDatabase().update(
                LocalChannelTable.TABLE_NAME,
                values,
                LocalChannelTable.COL_CHANNEL_ID.name() + " LIKE ? AND "
                        + LocalChannelTable.COL_STATE.name() + " <> ?",
                new String[]{BilibiliService.CHANNEL_PREFIX + "%", String.valueOf(Status.OK.code)});
        if (updated > 0) {
            Logger.i(this, "Restored %d stale Bilibili channel state(s)", updated);
        }
    }
