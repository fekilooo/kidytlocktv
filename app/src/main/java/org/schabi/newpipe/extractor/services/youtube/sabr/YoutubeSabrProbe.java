...
public final class YoutubeSabrProbe {
    ...
    static YoutubeSabrProbeResult postMediaRequest(
        ...
        try (StreamingResponse response = SkyTubeSabrHttp.postStreaming(
            ...
            if (response.responseCode() >= 400) {
                Log.w("YoutubeSabrProbe", "UMP HTTP " + response.responseCode() + " "
                        + streamed.getDecodedResponse().summarizeForDiagnostics());
                if (streamed.getDecodedResponse().getParts().isEmpty()) {
                    throw new SabrProtocolException("SABR request was rejected before protocol "
                            + "processing: status=" + response.responseCode());
                }
            }
    ...
    private static JsonObject fetchPlayerResponse(@Nonnull final String videoId,
                                                   ...
                                                   @Nullable final String visitorDataOverride)
            throws IOException, ExtractionException {
        final boolean streamExtractorMweb = profile == YoutubeSabrClientProfile.MWEB
                && (playerPoToken == null || playerPoToken.isEmpty())
                && (visitorDataOverride == null || visitorDataOverride.isEmpty());
        final byte[] body = streamExtractorMweb
                ? createStreamExtractorMwebPlayerBody(videoId, localization, contentCountry, cpn)
                : createPlayerBody(videoId, profile, localization, contentCountry,
        ...
        final Response response = NewPipe.getDownloader().post(url,
                streamExtractorMweb
                        ? buildStreamExtractorMwebPlayerHeaders(profile)
    ...
    @Nonnull
    private static byte[] createStreamExtractorMwebPlayerBody(
            @Nonnull final String videoId,
            @Nonnull final Localization localization,
            @Nonnull final ContentCountry contentCountry,
        ...
        return JsonWriter.string(JsonObject.builder()
                .object("context")
                    .object("client")
                        ...
                        .value("userAgent", YoutubeSabrClientProfile.MWEB.getUserAgent())
                        .value("clientName", YoutubeSabrClientProfile.MWEB.getClientName())
                        .value("clientVersion", YoutubeParsingHelper.getClientVersion())
                    .end()
                .end()
    ...
    private static Map<String, List<String>> buildPlayerHeaders(
        ...
        if (profile == YoutubeSabrClientProfile.ANDROID
            ...
            headers.putAll(YoutubeParsingHelper.getCookieHeader());
        }
        return headers;
    }

    @Nonnull
    private static Map<String, List<String>> buildStreamExtractorMwebPlayerHeaders(
            @Nonnull final YoutubeSabrClientProfile profile)
            throws IOException, ExtractionException {
        final Map<String, List<String>> headers = new HashMap<>();
        headers.put("Content-Type", Collections.singletonList("application/json"));
        ...
        headers.put("X-YouTube-Client-Name",
                Collections.singletonList(profile.getClientId()));
        headers.put("X-Youtube-Client-Version",
    ...
    static String maybeDeobfuscateNParameter(@Nonnull final String videoId,
        ...
        final java.util.regex.Matcher queryMatcher =
                java.util.regex.Pattern.compile("([?&])n=([^&]+)").matcher(url);
        if (queryMatcher.find()) {
            final String encryptedN = urlDecode(queryMatcher.group(2));
            try {
                final String decodedN = PipePipeNParameterDecoder.decode(encryptedN);
                return url.substring(0, queryMatcher.start(2)) + urlEncode(decodedN)
                        + url.substring(queryMatcher.end(2));
            } catch (final ParsingException pipePipeFailure) {
                Log.w("YoutubeSabrProbe", "PipePipe n decoder failed; trying legacy decoder",
                        pipePipeFailure);
            }
        }
        return YoutubeJavaScriptPlayerManager
